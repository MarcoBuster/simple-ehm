...
parser.add_argument(
    ...
    type=float,
    default=1,
    help="divide window size (default: 1s) by this factor",
)
parser.add_argument(
    "--window-slide-divide",
    type=float,
    default=2,
    help="divide the window slide by this factor (default: half the window size)",
)
parser.add_argument(
    "--fps",
    type=int,
    default=-1,
    help="frames per second of the encoded video. Lower FPS mean faster encoding (default: original)",
)
...
parser.add_argument(
    ...
    default=False,
    action="store_true",
    help="print spectrogram of window_size sliding by window_slide during analysis (debubbing only)",
)
parser.add_argument(
    "--generate-training-data",
    default=False,
    action="store_true",
    help="export extracted ehm(s) and silences as well to a separate folder. Useful for training on false positives",
)
parser.add_argument(
    "--srt",
    default=False,
    action="store_true",
    help="generate subtitle track for easier accuracy evaluation",
)
parser.add_argument(
    "--keep",
    nargs="+",
    default=["speech"],
    help="space separated tags to to be kept in the final video. Eg: ehm silence. Default: speech",
)
parser.add_argument(
    "--keep-junk", default=False, action="store_true", help="keeps tmp files"
)
...
def get_spectrogram(waveform, seek, window_size):
    ...
    spectrogram = tf.signal.stft(
        equal_length, frame_length=255, frame_step=128, pad_end=True
    )
...
def generate_tdata(ss, to, count, label):
    ...
    cuts[-1].extend(
        ["-c:a", "pcm_s16le", "-ac", "1", "-ar", "16000", "-filter:a", "dynaudnorm"]
    )
...
def analyze_track(model, waveform, sample_rate):
    ...
    if args.srt:
        sub = open(
            video_path[:-4] + ".srt", "w", encoding="utf-8"
        )  # subtitle track name
    else:
    ...
    for i in range(0, audio_len, window_slide):
        ...
        cv2.putText(
            img, labels[cls], (5, 120), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 255), 2
        )
...
if __name__ == "__main__":
    ...
    for i in range(len(labels)):
        ...
        print(
            ...
            "(s)",
            " per un ammontare di ",
            str(datetime.timedelta(seconds=stats[i][1]))[:8],
            sep="",
        )
